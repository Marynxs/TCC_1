...
quais combinações apresentam melhor desempenho com conjuntos limitados 
de imagens anotadas por especialistas. A justificativa baseia-se na 
...
imagens por configuração experimental, com anotações produzidas por 
avaliadores especializados. A avaliação adotará validação cruzada 
...
a detecção de defeitos sutis, técnicas de realce de contraste adaptativo têm 
sido aplicadas como etapa de pré-processamento, amplificando variações 
locais de intensidade independentemente das condições de iluminação \cite{Chen2021, Sawada2024}.
...
No contexto da inspeção visual automatizada, a combinação de filtros 
de gradiente direcional com técnicas de equalização de histograma 
tem demonstrado melhorar a acurácia de extração de defeitos em 
superfícies industriais. \citeonline{Sawada2024} propõem um método 
de inspeção de superfícies pintadas que aplica filtro \textit{emboss} 
seguido de equalização de histograma como pré-processamento, 
demonstrando que essa combinação melhora a detecção de defeitos em 
relação à abordagem convencional sem pré-processamento.

No contexto deste trabalho, as imagens fotométricas derivadas serão 
geradas pela aplicação sequencial do filtro emboss e do 
\textit{Contrast Limited Adaptive Histogram Equalization} (CLAHE) 
sobre as imagens RGB originais das cartas, utilizando a biblioteca 
OpenCV. O filtro emboss produz uma representação em tons de cinza 
que realça variações de relevo e textura superficial, enquanto o 
CLAHE amplifica os detalhes finos do relevo resultante sem saturar 
regiões homogêneas \cite{Jiang2023}. Essa representação será 
incorporada ao pipeline experimental como entrada alternativa, 
permitindo investigar se o realce de gradiente direcional de 
superfície contribui para a detecção de defeitos em cartas Pokémon 
TCG em cenário com baixo volume de dados.
...
originais, utilizando a biblioteca OpenCV, com parâmetros definidos 
nos testes preliminares previstos na 
