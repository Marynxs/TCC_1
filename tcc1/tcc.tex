...
captura das imagens: inconsistências espaciais de brilho causadas pela 
geometria do equipamento de captura, pelo ângulo da câmera e por 
reflexos ambientais podem obscurecer defeitos reais ou gerar artefatos 
visuais interpretados erroneamente como danos \cite{Zheng2021}. Esse tipo 
de não-uniformidade é amplamente reconhecido na área de visão 
computacional industrial \cite{Zheng2021}. Para mitigar seu impacto sobre 
a detecção de defeitos sutis, técnicas de realce de superfície baseadas 
em filtragem de gradiente têm sido aplicadas como etapa de 
pré-processamento, produzindo representações visuais que amplificam 
variações locais de textura e relevo independentemente das condições 
de iluminação \cite{Chen2021}.
...
exemplos rotulados, como o conjunto de 88 cartas disponível neste 
trabalho, caracterizam uma restrição real que demanda estratégias de 
treinamento específicas. 
...
diferentes estratégias de treinamento no desempenho do modelo. Não foram 
identificados na literatura estudos que avaliem o impacto relativo de 
estratégias de treinamento para detecção de defeitos em cartas TCG em 
cenários com conjuntos reduzidos de dados reais, lacuna que este 
trabalho se propõe a investigar sobre um conjunto de 88 cartas fornecido 
por uma empresa brasileira especializada em graduação. 
...
pequenas dobras \cite{Nahar2025}. Por esse motivo, a pesquisa
...
a identificação de defeitos sutis \cite{Zheng2021}. Para mitigar seu impacto sobre a detecção de defeitos sutis, técnicas 
de realce de contraste adaptativo têm sido aplicadas como etapa de 
pré-processamento, amplificando variações locais de intensidade 
independentemente das condições de iluminação \cite{Chen2021, Jiang2023}.

Entre essas técnicas, o \textit{Contrast Limited Adaptive Histogram 
Equalization} (CLAHE) destaca-se por realizar o realce de forma 
adaptativa por regiões da imagem, limitando a amplificação de ruído 
e equilibrando a distribuição de brilho sem saturar regiões com 
variações intensas de intensidade \cite{Jiang2023}. Embora o contexto 
de aplicação seja distinto, \citeonline{Jiang2023} demonstram que 
a aplicação de CLAHE como pré-processamento antes do treinamento de 
detectores baseados em YOLO produz ganhos consistentes de mAP em 
superfícies com variações irregulares de iluminação, princípio 
diretamente aplicável ao presente trabalho, no qual superfícies 
holográficas (\textit{foil}) introduzem reflexos intensos que 
comprometem a visibilidade de defeitos sutis na imagem RGB convencional.

No contexto deste trabalho, as imagens fotométricas derivadas serão 
geradas pela aplicação de CLAHE sobre as imagens RGB originais das 
cartas, utilizando a biblioteca OpenCV. Essa representação será 
incorporada ao pipeline experimental como entrada alternativa, 
permitindo investigar se o realce adaptativo de contraste contribui 
para a detecção de defeitos em cartas Pokémon TCG em cenário com 
...
fotométricas derivadas. Essas imagens serão geradas pela aplicação de CLAHE sobre as imagens 
RGB originais, com o objetivo de realçar variações locais de contraste 
e possíveis irregularidades superficiais, tornando defeitos sutis mais 
evidentes do que na imagem convencional \cite{Jiang2023}.
...
\cite{Nahar2025}. As regiões anotadas foram
...
Previamente à execução dos experimentos, será realizada uma análise 
descritiva do conjunto de dados, incluindo o número de cartas com 
pelo menos uma região defeituosa anotada e o número médio de anotações 
por imagem. Essa análise permitirá identificar eventuais desequilíbrios 
entre imagens com e sem defeitos anotados — situação comum em datasets 
de inspeção visual \cite{Chen2021} — e orientar ajustes nos testes 
preliminares caso necessário.
...
70 cartas de treinamento por rodada, aumentariam o risco de 
...
As imagens fotométricas derivadas serão geradas pela aplicação de 
CLAHE sobre as imagens RGB originais, utilizando a biblioteca OpenCV, 
com parâmetros definidos nos testes preliminares previstos na 
